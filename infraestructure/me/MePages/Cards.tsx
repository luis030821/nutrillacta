...
export interface cards {
  ...
  card: string;
  name: string;
  entity: "Visa" | "Amex" | "Mastercard" | "Unknown";
}

function Cards({ onlyModal, find }: { onlyModal?: boolean; find?: any }) {
  const modal = useModal();
  const [numberCard, setNumberCard] = useState<string>("");
  const [expirationDate, setExpirationDate] = useState<string>("");
  const [cvv, setCvv] = useState<string>();
  const [name, setName] = useState<string>("");
  const { cards, save, deleteCard } = useCards();
  const formatNumberCard = (value: string | undefined) => {
  ...
  const formatExpirationDate = (value: string | undefined) => {
    const newValue = value?.replace(/\D/g, "").slice(0, 4);
    return newValue?.replace(/(.{2})/g, "$1 ").trim();
  };
  const modalDelete = useModal();
  const [deleteInfo, setDeleteInfo] = useState<cards>();
  useCss(`
    .rccs {
      margin: 0 auto;
  ...
  return (
    <div className="w-full">
      <p className="text-gray-600 text-[.9rem]">
        Únicamente puedes eliminar o agregar nuevas tarjetas
      </p>
      <div className="flex justify-center mt-3">
        <button
          className="flex items-center justify-center gap-3 border border-blue-500   rounded-[6px] w-full py-3"
          onClick={modal.open}
        >
          <Icons className="stroke-blue-500" icon="IconCreditCard"></Icons>
          <span className="text-blue-500">Añadir tarjeta crédito/débito</span>
        </button>
      </div>
      {!onlyModal && (
      ...
      >
        <div className="bg-white w-full h-full">
          ...
          <div className="flex items-center justify-center w-full h-calc(100%_-_150px) ">
            <div className=" p-3 rounded-[10px] max-w-[370px]">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <div className="flex items-center justify-center">
                    <div className="flex flex-col w-full max-w-[370px]">
                      <div className=" w-full tarjeta-number border flex items-center">
                        <input
                          id="input-tarjeta"
                          value={formatNumberCard(numberCard)}
                      ...
                      <div className="flex mt-[-2px]">
                        <input
                          ...
                          placeholder="MM / AA"
                          className=" w-1/2 dia-mes border"
                          onChange={(e) => {
                ...
                <div className="w-full">
                  ...
                  <input
                    ...
                    type="text"
                    className="px-4 py-2 border border-[#e0e0e0] rounded-[10px] w-full"
                  />
                </div>
                <div>
                  <p className="text-[.8rem] text-gray-600">
                    Podrás eliminar tu tarjeta en cualquier momento. Al
                    continuar aceptas nuestras{" "}
                    <Link
                    ...
                    <Link
                      target="_blank"
                      className="underline"
                      href="/politicaPrivacidad?text=tarjetas"
                    >
                      {" "}
                ...
                <div className="flex justify-center mt-6">
                  <button
                    ...
                    onClick={async () => {
                      modal.close();
                      //@ts-ignore
                      await save(numberCard, expirationDate, cvv, name);
                      find?.();
                    }}
                  >
                    <Icons
                      ...
                      icon="IconCreditCard"
                    ></Icons>
                    <span className="text-blue-500">Añadir tarjeta</span>
                  </button>
                </div>
                {/* <div className="flex justify-center mt-6">
                  <button
                    className=" border border-[#e0e0e0] bg-paleta-100 text-white  rounded-[6px] w-full py-3"
                    onClick={async () => {
                      modal.close();
                      //@ts-ignore
                      await save(numberCard, expirationDate, cvv, name);
      ...
      <Modal showBottom={false} title="Eliminar tarjeta" modal={modalDelete}>
        <>
          <div className="relative bg-white rounded-lg ">
            <div className="p-4 md:p-5 text-center">
              <svg
                className="mx-auto mb-4 fill-none text-gray-400 w-12 h-12"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
              >
                <path
              ...
              <button
                ...
                type="button"
                className="py-2.5 mr-5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100"
              >
                No, cancelar
              </button>
              <button
                onClick={async () => {
                  modalDelete.close();
                  //@ts-ignore
                  await deleteCard(deleteInfo?.cardId);
                }}
                type="button"
